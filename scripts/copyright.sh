...
if command -v nwa &> /dev/null; then
  nwa add -l apache -c "IBM Corp." scripts tests src examples
elif command -v docker &> /dev/null; then
  docker run -it -v "${PWD}:/src" ghcr.io/b1nary-gr0up/nwa:main add -l apache -c "IBM Corp." scripts tests src examples
else
